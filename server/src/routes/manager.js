...
// Dashboard stats
router.get('/dashboard', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
...
// Get activity log
router.get('/activity', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
...
router.get('/locations', authenticate, authorize('manager', 'admin', 'employee'), async (req, res, next) => {
  try {
...
router.post('/locations', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
...
router.delete('/locations/:id', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
    const { id } = req.params;
...
router.post('/import', authenticate, authorize('manager', 'admin'), upload.single('file'), async (req, res, next) => {
  try {
    if (!req.file) {
...
router.get('/products', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
...
// Get marketplaces
router.get('/marketplaces', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
...
router.get('/diagnostics/duplicates', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
...
router.post('/diagnostics/cleanup', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
...
// Get single user
router.get('/users/:id', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
    const { id } = req.params;
    const result = await db.query(`
      SELECT u.id, u.email, u.name, u.role, u.client_id, u.is_active, u.created_at,
...
router.post('/users', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
    ...
    // Validate role - managers can only create employee/client users, admins can create any
    const validRoles = req.user.role === 'admin'
...
router.patch('/users/:id', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
    const { id } = req.params;
    ...
    if (role !== undefined) {
      // Validate role - managers can only set employee/client roles, admins can set any
      const validRoles = req.user.role === 'admin'
        ? ['admin', 'manager', 'employee', 'client']
        : ['employee', 'client'];
...
// Reset user password
router.post('/users/:id/reset-password', authenticate, authorize('manager', 'admin'), async (req, res, next) => {
  try {
    const { id } = req.params;
